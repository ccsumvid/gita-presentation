...
const renderer = (function() {
   ...
   * 18px), then wrapping is restored (graceful fallback if the floor is hit).
   *
   ...
   * Inside a hidden container all widths measure 0, making this a no-op.
   */
  function fitLines(target) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      while (el.scrollWidth > avail - 1 && size > 18 && guard++ < 30) {
        size -= 1;
